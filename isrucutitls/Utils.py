...
def ReadConfig(configfile):  #读取参数
...
def AddContext_MultiSub(x, y, Fold_Num, context, i):
    ...
    id_del = np.concatenate([np.cumsum(fold) - i for i in range(1, context)])  #np.cumsum求前缀和
...
#实例化操作
def Instantiation_optim(name, lr):  #优化器
    if   name=="adam":
...
def Instantiation_regularizer(l1, l2):  #正则化器
    if   l1!=0 and l2!=0:
...
def scaled_Laplacian(W):
    ...
    '''
    assert W.shape[0] == W.shape[1]  #图的邻接矩阵
    D = np.diag(np.sum(W, axis = 1))  #度矩阵
    L = D - W  #拉普拉斯矩阵
    lambda_max = eigs(L, k = 1, which = 'LR')[0].real  #L的最大特征值
    return (2 * L) / lambda_max - np.identity(W.shape[0])  #无向图的归一化拉普拉斯矩阵
